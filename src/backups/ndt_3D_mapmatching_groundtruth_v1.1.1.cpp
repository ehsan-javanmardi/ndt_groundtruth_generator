/*
 ...
 2017.10.27
 ndt_3D_mapmatching_groundtruth_v1.1.1
 ...
 CHANGE LOG :
    ndt_3D_mapmatching_groundtruth_v1.1.1
    Save ground truth to the CSV file not PCD file


 Previous change logs :

    Use new point type PointXYZIT doe delta_t and disotrtion
...
ros::Publisher best_transformed_scan_pub;
...
*/
            ...
            for (int y_iter=-4; y_iter < 4; y_iter++)
                //for (int yaw_iter=-4; yaw_iter<4; yaw_iter++) // in degree
                {
                    ...
                    if (max_trans_probability < iter_trans_probability)
                    {
                        ...
                        std::cout << "$$$$$$$$$$ BEST SCORE/#POINTS IS UPDATED. SCORE/#POINTS is " << max_trans_probability\
                                  << "$$$$$$$$$$" << std::endl;
    ...
    if (true)
    {
        ...
        publish_pointCloud(best_transformed_scan, best_transformed_scan_pub, "map");
    }
...
{
    std::cout << "ndt_3D_mapmatching_groundtruth_v1_1_1\n" ;
    ros::init(argc, argv, "ndt_3D_mapmatching_groundtruth_v1_1_1");
    ...
    best_transformed_scan_pub = nh.advertise<sensor_msgs::PointCloud2>("/best_transformed_scan", 1000);

    ndt_map_pub = nh.advertise<visualization_msgs::MarkerArray>("visualization_marker_array", 10000);

    ros::Subscriber scan_sub = nh.subscribe("velodyne_points", 10000, scan_callback);
